            ...
            <table width="100%">
              <tbody>
                ...
                <?php
                  $rows=$Mvim->all();
                  foreach ($rows as $row){
                    printf('<tr><td><img src="../icon/%s" style="width:100px;height:100px"></td>',$row['img']);
                    ...
                    printf('<td><input type="button" onclick="op(\'%s\',\'%s\',\'%s\')" value="更換動畫"><input type="hidden" name="id[]" value="%s"></td></tr>','#cover','#cvr','../window/mvim.php?id='.$row['id'],$row['id']);
                  }
                ?>
              </tbody>
            </table>
